...
1. **Establish scope.** Which API operation(s), which Mongo collection(s),
   ...
   This gate has no default; see `references/clarifying-questions.md` item 1.
2. **Discover the API side.** Internal: scan this repository per
   `references/api-discovery.md` — prefer a committed OpenAPI/Swagger file,
   ...
   cannot read.
3. **Discover the MongoDB side.** Per `references/mongo-discovery.md`: prefer
...
4. **Ask the clarifying questions.** The gate list is
   `references/clarifying-questions.md`. What actually gets persisted, the
...
5. **Write the mapping document** and have it reviewed. Format and required
   sections: `references/mapping-document-format.md`.
...
Read `references/api-discovery.md` in full before doing this. In short:
...
Read `references/mongo-discovery.md` in full before doing this. In short,
...
Work through `references/clarifying-questions.md`. Ask in one grouped batch
...
Full structure and a worked template: `references/mapping-document-format.md`.
